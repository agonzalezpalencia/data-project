...
# Método que pinta en Streamlit la primer métrica del ejercicio 4
def exercise4_metric_1():

    # Declaramos título de la página
    st.title(f'Reviews y satisfacción del cliente\n' +
                '## Ejercicio 4.1\n - Número de reviews por estado (Estado de Ciudad y Estado de Pedido "delivered")')
    
    # Declaramos el input númerico, declaramos la métrica en base a ese input y representamos la métrica con un gráfico de barras
    ...
    # Declaramos el input númerico y declaramos la métrica en base a ese input
    ...
    # Creamos tantas columnas como longitud tenga nuestro dataframe
    columns = st.columns(len(ex4_1_metric_2))

    # Recorremos las columnas
    for i in range(0, len(ex4_1_metric_2)):

        # Sacamos un metric de streamlit para cada columna
        with columns[i]:
